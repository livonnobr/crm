    ...
    <div className="pt-[68px] md:pt-0">
      ...
      <div className="px-5 pb-12 md:px-10">
        <section className="instrument-strip" aria-label="Instrumentos de acompanhamento comercial">
          ...
          <div className="instrument-cell relative isolate overflow-hidden border-[#E5BE6B] bg-[#FFF4D9] shadow-[0_10px_24px_rgba(196,141,35,0.12)]"><div className="pointer-events-none absolute inset-x-5 bottom-4 top-24 -z-10 opacity-25"><div className="grid grid-cols-7 gap-x-3 gap-y-2 text-center text-[12px] font-bold text-[#C99D4B]">{["S", "T", "Q", "Q", "S", "S", "D"].map((day, index) => <span key={`weekday-${index}`} className="text-[9px] uppercase tracking-[0.12em] text-[#B78B38]">{day}</span>)}{calendarDays.map((day, index) => <span key={`day-${index}`} className={`rounded-md py-1 ${day === new Date().getDate() && selectedMonth === new Date().toISOString().slice(0, 7) ? "bg-[#E5BE6B] text-[#6B4D12]" : ""}`}>{day ?? ""}</span>)}</div></div><div className="relative z-10"><div className="flex items-center gap-2"><span className="pulse-dot bg-[#C88920]" /><span className="text-[10px] font-extrabold uppercase tracking-[0.11em] text-[#8A651D]">Fechamento do mês</span></div><p className="mt-3 font-display text-[38px] font-extrabold tracking-[-0.07em] text-[#6B4D12]">{daysRemaining} {daysRemaining === 1 ? "dia útil" : "dias úteis"}</p><p className="mt-2 text-[14px] font-semibold capitalize text-[#8A6D2A]">restantes em {currentMonth}</p></div></div>
        </section>

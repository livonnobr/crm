    ...
    <div className="pt-[68px] md:pt-0">
      ...
      <div className="px-5 pb-12 md:px-10">
        <section className="instrument-strip" aria-label="Instrumentos de acompanhamento comercial">
          ...
          <div className="instrument-cell min-w-0 border-[#E5BE6B] bg-[#FFF4D9] shadow-[0_10px_24px_rgba(196,141,35,0.12)]"><div className="flex items-center gap-2"><span className="pulse-dot bg-[#C88920]" /><span className="text-[9px] font-extrabold uppercase tracking-[0.1em] text-[#8A651D]">Fechamento do mês</span></div><p className="mt-2 font-display text-[30px] font-extrabold leading-none tracking-[-0.065em] text-[#6B4D12]">{daysRemaining} {daysRemaining === 1 ? "dia útil" : "dias úteis"}</p><p className="mt-1 text-[11px] font-semibold capitalize text-[#8A6D2A]">restantes em {currentMonth}</p><div className="mt-4 rounded-xl border border-[#E7C983] bg-[#FFF8E8] p-3"><div className="grid grid-cols-7 gap-x-2 gap-y-1.5 text-center text-[12px] font-bold text-[#A87520]"><div className="contents">{["S", "T", "Q", "Q", "S", "S", "D"].map((day, index) => <span key={`weekday-${index}`} className={`text-[9px] uppercase tracking-[0.12em] ${index >= 5 ? "text-[#B04D45]" : "text-[#8C641D]"}`}>{day}</span>)}</div>{calendar.days.map((day, index) => {
                ...
                return <span key={`day-${index}`} title={isHoliday ? "Feriado nacional" : isWeekend ? "Fim de semana" : undefined} className={`rounded-md py-1 ${isToday ? isRedDay ? "bg-[#FCE7E7] text-[#A33D3D] ring-1 ring-[#D97373]" : "bg-[#E5BE6B] text-[#5E4310] shadow-sm" : isRedDay ? "font-extrabold text-[#B04D45]" : "text-[#A87520]"}`}>{day ?? ""}</span>;
              })}</div></div></div>
        </section>
...
function GoalSimulator({ stages, services, funnels, onAddStage, onUpdateStage, onDeleteStage, onSave, isSaving, syncConfirmed }: { stages: GoalSimulationStage[]; services: Service[]; funnels: SalesFunnel[]; onAddStage: () => void; onUpdateStage: (id: string, field: "name" | "color" | "probability" | "projectionMode" | "fixedValue" | "productId" | "conversionStageId", value: string | number) => void; onDeleteStage: (id: string) => void; onSave: () => void | Promise<void>; isSaving: boolean; syncConfirmed: boolean }) {
  ...
  return <section className="surface-panel mt-5 overflow-hidden p-4 sm:p-6">
    <div className="flex flex-col gap-4 border-b border-[#E8ECE6] pb-5 lg:flex-row lg:items-end lg:justify-between">
      ...
      <div className="flex flex-wrap items-end gap-3"><label className="block"><span className="mb-1 block text-[10px] font-extrabold uppercase tracking-[0.12em] text-[#74827B]">Entradas no cenário</span><Input min="0" step="1" type="number" value={leadInput} onChange={(event) => setLeadInput(Number(event.target.value))} className="h-10 w-[154px] bg-white font-bold" /></label><Button type="button" onClick={onAddStage} className="h-10 gap-2 rounded-xl bg-[#18201E] px-4 text-xs font-extrabold text-white hover:bg-[#087E5A]"><CirclePlus size={15} />Adicionar etapa</Button><Button type="button" onClick={() => void onSave()} disabled={isSaving} className="h-10 gap-2 rounded-xl bg-[#10A97A] px-4 text-xs font-extrabold text-white hover:bg-[#087E5A] disabled:cursor-wait disabled:opacity-70">{isSaving ? <RotateCcw className="h-3.5 w-3.5 animate-spin" /> : <ClipboardCheck size={15} />}{isSaving ? "Salvando..." : "Salvar metas"}</Button><span className={`inline-flex h-10 items-center rounded-xl border px-3 text-[10px] font-extrabold uppercase tracking-[0.08em] ${syncConfirmed ? "border-[#CBE5D7] bg-[#F0F8F3] text-[#087E5A]" : "border-[#E7D5A8] bg-[#FFF9E8] text-[#80621D]"}`}>{syncConfirmed ? "Sincronizado com Supabase" : "Alterações não salvas"}</span></div>
    </div>
    <div className="mt-5 grid gap-4 xl:grid-cols-[minmax(0,1.15fr)_minmax(320px,0.85fr)]">
      <div className="rounded-2xl border border-[#E1EAE2] bg-[#F8FBF8] p-3 sm:p-4">
        <div className="mb-3 flex items-center justify-between gap-3 px-1"><div><p className="text-[10px] font-extrabold uppercase tracking-[0.14em] text-[#6D8176]">Configuração</p><p className="mt-0.5 text-sm font-bold text-[#27302D]">Defina como cada etapa será projetada</p></div><span className="rounded-full bg-white px-2.5 py-1 text-[10px] font-extrabold text-[#6C8177]">{stages.length} etapas</span></div>
        <div className="space-y-3">{stages.map((stage, index) => {
          ...
          return <article key={stage.id} className="rounded-2xl border border-[#E1E8E1] bg-white p-4 shadow-[0_7px_20px_rgba(35,65,50,0.04)] transition hover:border-[#BFD8C9] hover:shadow-[0_12px_26px_rgba(35,65,50,0.07)]">
            <div className="flex items-start gap-3"><span className="mt-1 h-3 w-3 shrink-0 rounded-full ring-4 ring-[#F1F6F1]" style={{ backgroundColor: stage.color }} /><div className="min-w-0 flex-1">{isEditing ? <Input autoFocus value={nameDraft} onChange={(event) => setNameDraft(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") saveName(stage); if (event.key === "Escape") setEditingStageId(null); }} className="h-9 bg-white text-sm font-bold" /> : <div className="flex flex-wrap items-center gap-2"><h3 className="truncate font-display text-base font-extrabold tracking-[-0.025em] text-[#27302D]">{stage.name}</h3>{isWonStage(stage) && <span className="rounded-full bg-[#E7F5EF] px-2 py-1 text-[9px] font-extrabold uppercase tracking-[0.08em] text-[#087E5A]">Fechamento</span>}</div>}<p className="mt-1 text-xs font-medium text-[#829089]">{index === 0 ? "Ponto de partida do cenário" : "Configure a regra de projeção desta etapa"}</p></div><div className="flex shrink-0 items-center gap-1"><button type="button" onClick={() => isEditing ? saveName(stage) : startEditing(stage)} className="grid h-8 w-8 place-items-center rounded-lg text-[#718078] transition hover:bg-[#E8F6F0] hover:text-[#087E5A]" title={isEditing ? "Salvar nome" : "Editar etapa"} aria-label={isEditing ? `Salvar nome de ${stage.name}` : `Editar ${stage.name}`}>{isEditing ? <CheckCircle2 size={15} /> : <Pencil size={15} />}</button>{isEditing && <button type="button" onClick={() => setEditingStageId(null)} className="grid h-8 w-8 place-items-center rounded-lg text-[#718078] hover:bg-[#F6EAE8] hover:text-[#B44D46]" title="Cancelar edição" aria-label="Cancelar edição"><X size={15} /></button>}<button type="button" onClick={() => onDeleteStage(stage.id)} disabled={stages.length <= 2} className="grid h-8 w-8 place-items-center rounded-lg text-[#9C6D69] transition hover:bg-[#F6EAE8] hover:text-[#B44D46] disabled:cursor-not-allowed disabled:opacity-30" title={stages.length <= 2 ? "Mantenha pelo menos duas etapas" : "Excluir etapa"} aria-label={`Excluir ${stage.name}`}><Trash2 size={15} /></button></div></div>
            {index === 0 ? <div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-[#DDEBE1] bg-[#F4FAF6] px-3 py-2.5"><div className="flex items-center gap-2"><input type="color" value={stage.color} onChange={(event) => onUpdateStage(stage.id, "color", event.target.value)} className="h-8 w-10 cursor-pointer rounded-lg border border-[#D7E4DA] bg-white p-0.5" aria-label={`Cor de ${stage.name}`} /><span className="text-xs font-semibold text-[#5F7468]">Entrada fixa em 100%</span></div><strong className="font-display text-lg tracking-[-0.04em] text-[#087E5A]">{formatSimulationValue(leads)} leads</strong></div> : <>
              <div className="mt-4 grid gap-3 sm:grid-cols-[minmax(150px,0.42fr)_minmax(0,1fr)]"><label className="block"><span className="mb-1.5 block text-[10px] font-extrabold uppercase tracking-[0.12em] text-[#73837A]">Método de projeção</span><select value={mode} onChange={(event) => onUpdateStage(stage.id, "projectionMode", event.target.value as SimulationProjectionMode)} className="h-10 w-full rounded-xl border border-[#DCE6DD] bg-white px-3 text-sm font-bold text-[#34423B] outline-none transition focus:border-[#10A97A] focus:ring-2 focus:ring-[#10A97A]/15"><option value="percentual">Percentual</option><option value="fixo">Número fixo</option><option value="produto">Produto × conversão</option></select></label><div className="rounded-xl border border-[#E5EBE5] bg-[#FBFCFA] p-3">{mode === "percentual" && <label className="block"><span className="mb-1.5 block text-[10px] font-extrabold uppercase tracking-[0.12em] text-[#73837A]">Conversão sobre a etapa anterior</span><div className="relative"><Input min="0" max="100" step="0.1" type="number" value={stage.probability} onChange={(event) => onUpdateStage(stage.id, "probability", event.target.value)} className="h-10 bg-white pr-8 text-right font-extrabold" /><span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-xs font-bold text-[#7C8A83]">%</span></div></label>}{mode === "fixo" && <label className="block"><span className="mb-1.5 block text-[10px] font-extrabold uppercase tracking-[0.12em] text-[#73837A]">Número fixo nesta etapa</span><Input min="0" step="1" type="number" value={stage.fixedValue ?? 0} onChange={(event) => onUpdateStage(stage.id, "fixedValue", event.target.value)} className="h-10 bg-white text-right font-extrabold" /></label>}{mode === "produto" && <div className="grid gap-3 sm:grid-cols-2"><label className="block"><span className="mb-1.5 block text-[10px] font-extrabold uppercase tracking-[0.12em] text-[#73837A]">Produto cadastrado</span><select value={stage.productId ?? ""} onChange={(event) => onUpdateStage(stage.id, "productId", event.target.value)} disabled={!services.length} className="h-10 w-full rounded-xl border border-[#DCE6DD] bg-white px-3 text-sm font-semibold text-[#34423B] outline-none focus:border-[#10A97A] focus:ring-2 focus:ring-[#10A97A]/15 disabled:bg-[#F0F3F0]"><option value="">{services.length ? "Selecione um serviço" : "Cadastre um serviço primeiro"}</option>{services.map((service) => <option key={service.id} value={service.id}>{service.name} · {formatCurrency(service.price)}</option>)}</select></label><label className="block"><span className="mb-1.5 block text-[10px] font-extrabold uppercase tracking-[0.12em] text-[#73837A]">Conversão da etapa</span><select value={stage.conversionStageId ?? ""} onChange={(event) => onUpdateStage(stage.id, "conversionStageId", event.target.value)} disabled={!funnelStages.length} className="h-10 w-full rounded-xl border border-[#DCE6DD] bg-white px-3 text-sm font-semibold text-[#34423B] outline-none focus:border-[#10A97A] focus:ring-2 focus:ring-[#10A97A]/15 disabled:bg-[#F0F3F0]"><option value="">{funnelStages.length ? "Selecione uma etapa" : "Cadastre uma etapa no funil"}</option>{funnelStages.map((funnelStage) => <option key={funnelStage.id} value={funnelStage.id}>{funnelStage.funnelName} · {funnelStage.name} · {funnelStage.probability}%</option>)}</select></label></div>}</div></div>
              <div className="mt-3 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-[#E8EDE8] bg-[#F8FAF8] px-3 py-2.5"><div className="flex min-w-0 items-center gap-2"><input type="color" value={stage.color} onChange={(event) => onUpdateStage(stage.id, "color", event.target.value)} className="h-8 w-10 cursor-pointer rounded-lg border border-[#D7E4DA] bg-white p-0.5" aria-label={`Cor de ${stage.name}`} /><div className="min-w-0"><p className="text-[10px] font-extrabold uppercase tracking-[0.12em] text-[#85928B]">Resultado estimado</p><p className="truncate text-xs font-medium text-[#6E7D75]">{mode === "produto" ? `${selectedProduct?.name ?? "Produto não selecionado"} · ${selectedConversionStage ? `${selectedConversionStage.name} ${selectedConversionStage.probability}%` : "Etapa não selecionada"}` : mode === "fixo" ? "Número informado manualmente" : `${stage.probability}% da etapa anterior`}</p></div></div><strong className="font-display text-lg tracking-[-0.04em] text-[#087E5A]">{projection?.displayUnit === "currency" ? formatCurrency(projection.projected) : `${formatSimulationValue(projection?.projected ?? 0)} leads`}</strong></div>
            </>}
          </article>;
        })}</div>
        <p className="mt-3 px-1 text-[11px] leading-5 text-[#829089]">A primeira etapa é a entrada e permanece em 100%. Use percentual para encadear volumes, número fixo para definir uma quantidade manual e produto × conversão para estimar receita com base no catálogo de Serviços e numa etapa do funil.</p>
      </div>
      <aside className="rounded-2xl border border-[#CFE3D6] bg-gradient-to-br from-[#EEF9F2] to-[#F8FCF8] p-4 sm:p-5"><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-extrabold uppercase tracking-[0.14em] text-[#5D7569]">Leitura projetada</p><h3 className="mt-1 font-display text-lg font-extrabold tracking-[-0.035em] text-[#24332D]">Cenário independente</h3></div><span className="rounded-full bg-white/80 px-2.5 py-1 text-[10px] font-extrabold uppercase tracking-[0.1em] text-[#087E5A]">{numberFormat.format(leads)} entradas</span></div><div className="mt-4 space-y-2">{stages.map((stage, index) => { const projection = projectionByStage.get(stage.id); if (!projection) return null; return <div key={stage.id} className="flex items-center gap-3 rounded-xl border border-white/80 bg-white/55 px-3 py-2.5"><span className="h-2.5 w-2.5 shrink-0 rounded-full" style={{ backgroundColor: stage.color }} /><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold text-[#4B6257]">{stage.name}</p><p className="truncate text-[10px] font-semibold uppercase tracking-[0.08em] text-[#87968E]">{index === 0 ? "Base" : stage.projectionMode === "produto" ? "Receita estimada" : stage.projectionMode === "fixo" ? "Número fixo" : `${stage.probability}% de conversão`}</p></div>{isWonStage(stage) && <span className="hidden rounded-full bg-[#E7F5EF] px-2 py-1 text-[9px] font-extrabold uppercase tracking-[0.08em] text-[#087E5A] sm:inline-flex">Final</span>}<strong className="font-display text-base tracking-[-0.04em] text-[#1B2522]">{projection.displayUnit === "currency" ? formatCurrency(projection.projected) : formatSimulationValue(projection.projected)}</strong></div>; })}</div><div className="mt-4 border-t border-[#CFE0D5] pt-4"><p className="text-xs font-bold text-[#4F6F60]">Os métodos podem ser combinados</p><p className="mt-1 text-xs leading-5 text-[#71887D]">Cada etapa tem sua própria regra. O resultado em moeda aparece apenas quando você escolhe Produto × conversão.</p></div></aside>
    </div>
  </section>;
}

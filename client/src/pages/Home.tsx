...
function cleanGoalPeriod(period: string) {
  return period.replace(/^(Diária|Semanal|Mensal)\s*[·|-]\s*/i, "").trim();
}

function goalPeriodValue(goal: GoalItem) {
  return `${goal.cadence ?? parseGoalCadence(goal.period)} · ${cleanGoalPeriod(goal.period)}`;
}
      ...
      <main className="min-h-screen min-w-0 flex-1">
        {page === "goals" ? (
          ...
          <PipelineWorkspace
            ...
            onDeleteDeal={(deal) => { if (window.confirm(`Excluir a oportunidade “${deal.title}”?`)) void deleteDeal(deal.id); }}
...
function PipelineColumn({ stage, deals, isOver, draggedDealId, onEditStage, onEditDeal, onDeleteDeal, onDragStart, onDragOver, onDrop, onDragEnd }: { stage: Stage; deals: Deal[]; isOver: boolean; draggedDealId: string | null; onEditStage: () => void; onEditDeal: (deal: Deal) => void; onDeleteDeal: (deal: Deal) => void; onDragStart: (event: DragEvent<HTMLElement>, dealId: string) => void; onDragOver: (event: DragEvent<HTMLElement>, stageId: string) => void; onDrop: (stageId: string, event?: DragEvent<HTMLElement>) => void; onDragEnd: () => void }) {

    ...
    <div className="pt-[68px] md:pt-0">
      ...
      <div className="px-5 pb-12 md:px-10">
        <section className="instrument-strip" aria-label="Instrumentos de acompanhamento comercial">
          ...
          <div className="instrument-cell min-w-0 border-[#E5BE6B] bg-[#FFF4D9] !p-3 shadow-[0_10px_24px_rgba(196,141,35,0.12)] sm:col-span-2 xl:col-span-2"><div className="flex items-center gap-2"><span className="pulse-dot bg-[#C88920]" /><span className="text-[9px] font-extrabold uppercase tracking-[0.1em] text-[#8A651D]">Fechamento do mês</span></div><p className="mt-1 font-display text-[24px] font-extrabold leading-none tracking-[-0.065em] text-[#6B4D12]">{daysRemaining} {daysRemaining === 1 ? "dia útil" : "dias úteis"}</p><p className="mt-0.5 text-[10px] font-semibold capitalize text-[#8A6D2A]">restantes em {currentMonth}</p><div className="mt-2 rounded-lg border border-[#E7C983] bg-[#FFF8E8] p-2"><div className="grid grid-cols-7 gap-x-1 gap-y-0 text-center text-[10px] font-bold text-[#A87520]"><div className="contents">{["S", "T", "Q", "Q", "S", "S", "D"].map((day, index) => <span key={`weekday-${index}`} className={`text-[9px] uppercase tracking-[0.12em] ${index >= 5 ? "font-black text-[#C62828]" : "text-[#8C641D]"}`}>{day}</span>)}</div>{calendar.days.map((day, index) => {
                ...
                return <span key={`day-${index}`} title={isHoliday ? "Feriado nacional" : isWeekend ? "Fim de semana" : undefined} className={`rounded-md py-0.5 ${isToday ? isRedDay ? "bg-[#C62828] font-black text-white ring-1 ring-[#A61B1B] shadow-sm" : "bg-[#E5BE6B] text-[#5E4310] shadow-sm" : isRedDay ? "bg-[#FFE6E3] font-black text-[#B42318]" : "text-[#A87520]"}`}>{day ?? ""}</span>;
              })}</div></div></div>
        </section>

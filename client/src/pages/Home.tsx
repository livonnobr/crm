...
import { persistenceLabel } from "@/lib/persistence-status";
    ...
    <div className="pt-[68px] md:pt-0">
      ...
      <div className="px-5 pb-12 md:px-10">
        <section className="instrument-strip" aria-label="Instrumentos de acompanhamento comercial">
          ...
          <div className="instrument-cell relative isolate min-h-[192px] overflow-hidden border-[#E5BE6B] bg-[#FFF4D9] shadow-[0_10px_24px_rgba(196,141,35,0.12)]"><div className="pointer-events-none absolute inset-x-4 bottom-3 top-[88px] z-0 rounded-xl border border-[#E7C983] bg-[#FFF8E8]/80 p-3"><div className="grid grid-cols-7 gap-x-2 gap-y-1.5 text-center text-[12px] font-bold text-[#A87520]">{["S", "T", "Q", "Q", "S", "S", "D"].map((day, index) => <span key={`weekday-${index}`} className="text-[9px] uppercase tracking-[0.12em] text-[#8C641D]">{day}</span>)}{calendarDays.map((day, index) => <span key={`day-${index}`} className={`rounded-md py-1 ${day === new Date().getDate() && selectedMonth === new Date().toISOString().slice(0, 7) ? "bg-[#E5BE6B] text-[#5E4310] shadow-sm" : "text-[#A87520]"}`}>{day ?? ""}</span>)}</div></div><div className="relative z-10"><div className="flex items-center gap-2"><span className="pulse-dot bg-[#C88920]" /><span className="text-[9px] font-extrabold uppercase tracking-[0.1em] text-[#8A651D]">Fechamento do mês</span></div><p className="mt-2 font-display text-[30px] font-extrabold leading-none tracking-[-0.065em] text-[#6B4D12]">{daysRemaining} {daysRemaining === 1 ? "dia útil" : "dias úteis"}</p><p className="mt-1 text-[11px] font-semibold capitalize text-[#8A6D2A]">restantes em {currentMonth}</p></div></div>
        </section>
...
function GoalSimulator({ stages, onAddStage, onUpdateStage, onDeleteStage, onSave, isSaving, syncConfirmed }: { stages: GoalSimulationStage[]; onAddStage: () => void; onUpdateStage: (id: string, field: "name" | "color" | "probability", value: string | number) => void; onDeleteStage: (id: string) => void; onSave: () => void | Promise<void>; isSaving: boolean; syncConfirmed: boolean }) {
  const [simulationMode, setSimulationMode] = useState<"quantity" | "value">("quantity");
  ...
  const simulationValue = Math.max(0, Number(valueInput) || 0);
  const simulationBase = simulationMode === "value" ? simulationValue : leads;
  ...
  const formatSimulationValue = (value: number) => simulationMode === "value" ? currencyFormat.format(value) : numberFormat.format(value);
  ...
  const projections = projectFunnelStages(stages, simulationBase, Object.fromEntries(stages.map((stage) => [stage.id, stage.probability])), isWonStage, isLostStage);
  ...
  return <section className="surface-panel mt-5 overflow-hidden p-4 sm:p-6">
    <div className="flex flex-col gap-4 border-b border-[#E8ECE6] pb-5 lg:flex-row lg:items-end lg:justify-between">
      ...
      <div className="flex flex-wrap items-end gap-3"><div><span className="mb-1 block text-[10px] font-extrabold uppercase tracking-[0.12em] text-[#74827B]">Projetar por</span><div className="flex h-10 rounded-xl border border-[#DDE5DE] bg-white p-1"><button type="button" onClick={() => setSimulationMode("quantity")} className={`rounded-lg px-3 text-xs font-extrabold transition ${simulationMode === "quantity" ? "bg-[#E8F6F0] text-[#087E5A]" : "text-[#7A8981] hover:text-[#087E5A]"}`}>Quantidade</button><button type="button" onClick={() => setSimulationMode("value")} className={`rounded-lg px-3 text-xs font-extrabold transition ${simulationMode === "value" ? "bg-[#E8F6F0] text-[#087E5A]" : "text-[#7A8981] hover:text-[#087E5A]"}`}>Valor</button></div></div><label className="block"><span className="mb-1 block text-[10px] font-extrabold uppercase tracking-[0.12em] text-[#74827B]">{simulationMode === "value" ? "Valor de entrada" : "Leads de entrada"}</span><div className="relative"><Input min="0" step={simulationMode === "value" ? "100" : "1"} type="number" value={simulationMode === "value" ? valueInput : leadInput} onChange={(event) => simulationMode === "value" ? setValueInput(Number(event.target.value)) : setLeadInput(Number(event.target.value))} className={`h-10 w-[154px] bg-white font-bold ${simulationMode === "value" ? "pl-8" : ""}`} />{simulationMode === "value" && <span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-xs font-extrabold text-[#74827B]">R$</span>}</div></label><Button type="button" onClick={onAddStage} className="h-10 gap-2 rounded-xl bg-[#18201E] px-4 text-xs font-extrabold text-white hover:bg-[#087E5A]"><CirclePlus size={15} />Adicionar etapa</Button><Button type="button" onClick={() => void onSave()} disabled={isSaving} className="h-10 gap-2 rounded-xl bg-[#10A97A] px-4 text-xs font-extrabold text-white hover:bg-[#087E5A] disabled:cursor-wait disabled:opacity-70">{isSaving ? <RotateCcw className="h-3.5 w-3.5 animate-spin" /> : <ClipboardCheck size={15} />}{isSaving ? "Salvando..." : "Salvar metas"}</Button><span className={`inline-flex h-10 items-center rounded-xl border px-3 text-[10px] font-extrabold uppercase tracking-[0.08em] ${syncConfirmed ? "border-[#CBE5D7] bg-[#F0F8F3] text-[#087E5A]" : "border-[#E7D5A8] bg-[#FFF9E8] text-[#80621D]"}`}>{syncConfirmed ? "Sincronizado com Supabase" : "Alterações não salvas"}</span></div>
    </div>
    <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1.05fr)_minmax(320px,0.95fr)]">
      <div><div className="space-y-2">{stages.map((stage, index) => { const projection = index === 0 ? { rate: 100, projected: simulationBase } : projectionByStage.get(stage.id); const isEditing = editingStageId === stage.id; return <div key={stage.id} className="rounded-xl border border-[#E7EBE6] bg-[#FCFCFA] px-3 py-2.5"><div className="flex items-center gap-2.5"><span className="h-2.5 w-2.5 shrink-0 rounded-full" style={{ backgroundColor: stage.color }} />{isEditing ? <Input autoFocus value={nameDraft} onChange={(event) => setNameDraft(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") saveName(stage); if (event.key === "Escape") setEditingStageId(null); }} className="h-8 min-w-0 flex-1 bg-white text-sm font-bold" /> : <p className="min-w-0 flex-1 truncate text-sm font-bold text-[#35403B]">{stage.name}</p>}{isWonStage(stage) && <span className="shrink-0 rounded-full bg-[#E7F5EF] px-2 py-1 text-[9px] font-extrabold uppercase tracking-[0.08em] text-[#087E5A]">Fechamento</span>}<button type="button" onClick={() => isEditing ? saveName(stage) : startEditing(stage)} className="grid h-8 w-8 place-items-center rounded-lg text-[#718078] hover:bg-[#E8F6F0] hover:text-[#087E5A]" title={isEditing ? "Salvar nome" : "Editar etapa"} aria-label={isEditing ? `Salvar nome de ${stage.name}` : `Editar ${stage.name}`}>{isEditing ? <CheckCircle2 size={15} /> : <Pencil size={15} />}</button>{isEditing && <button type="button" onClick={() => setEditingStageId(null)} className="grid h-8 w-8 place-items-center rounded-lg text-[#718078] hover:bg-[#F6EAE8] hover:text-[#B44D46]" title="Cancelar edição" aria-label="Cancelar edição"><X size={15} /></button>}<button type="button" onClick={() => onDeleteStage(stage.id)} disabled={stages.length <= 2} className="grid h-8 w-8 place-items-center rounded-lg text-[#9C6D69] hover:bg-[#F6EAE8] hover:text-[#B44D46] disabled:cursor-not-allowed disabled:opacity-30" title={stages.length <= 2 ? "Mantenha pelo menos duas etapas" : "Excluir etapa"} aria-label={`Excluir ${stage.name}`}><Trash2 size={15} /></button></div><div className="mt-2 flex items-center gap-3 pl-5"><label className="flex items-center gap-2 text-[11px] font-semibold text-[#7A8981]"><span>Cor</span><input type="color" value={stage.color} onChange={(event) => onUpdateStage(stage.id, "color", event.target.value)} className="h-7 w-9 cursor-pointer rounded-md border border-[#DDE5DE] bg-white p-0.5" aria-label={`Cor de ${stage.name}`} /></label><label className="relative ml-auto w-[92px]"><span className="sr-only">Taxa para {stage.name}</span><Input min="0" max="100" step="0.1" type="number" value={index === 0 ? 100 : stage.probability} disabled={index === 0} onChange={(event) => onUpdateStage(stage.id, "probability", event.target.value)} className="h-9 bg-white pr-7 text-right font-extrabold disabled:bg-[#F0F4F0] disabled:text-[#087E5A]" /><span className="pointer-events-none absolute right-2 top-1/2 -translate-y-1/2 text-xs font-bold text-[#7C8A83]">%</span></label><span className="text-[11px] text-[#8A9690]">{index === 0 ? "Base" : `${stage.probability}% da etapa anterior`}</span></div></div>; })}</div><p className="mt-3 text-[11px] leading-5 text-[#88958E]">A primeira etapa é a entrada e permanece em 100%. As taxas seguintes são editáveis e ficam salvas apenas nesta simulação.</p></div>
      <div className="rounded-2xl border border-[#D5E3DA] bg-[#F0F8F3] p-4 sm:p-5"><div className="flex items-center justify-between gap-3"><div><p className="text-[10px] font-extrabold uppercase tracking-[0.14em] text-[#5D7569]">Leitura projetada</p><p className="mt-1 text-sm font-bold text-[#27302D]">Simulação independente</p></div><span className="rounded-full bg-white/80 px-2.5 py-1 text-[10px] font-extrabold uppercase tracking-[0.1em] text-[#087E5A]">{simulationMode === "value" ? currencyFormat.format(simulationValue) : `${numberFormat.format(leads)} leads`}</span></div><div className="mt-4 space-y-2">{stages.map((stage, index) => { const projection = index === 0 ? { projected: simulationBase } : projectionByStage.get(stage.id); if (!projection) return null; return <div key={stage.id} className="flex items-center gap-3"><div className="flex w-5 justify-center"><span className="h-2 w-2 rounded-full" style={{ backgroundColor: stage.color }} /></div><p className="min-w-0 flex-1 truncate text-sm font-semibold text-[#53665D]">{stage.name}</p>{isWonStage(stage) && <span className="rounded-full bg-white/80 px-2 py-1 text-[9px] font-extrabold uppercase tracking-[0.08em] text-[#087E5E]">Etapa final</span>}<strong className="font-display text-lg tracking-[-0.04em] text-[#1B2522]">{formatSimulationValue(projection.projected)}</strong>{index < stages.length - 1 && <span className="sr-only">evolui para a próxima etapa</span>}</div>; })}</div><div className="mt-5 border-t border-[#CFE0D5] pt-4"><p className="text-xs font-bold text-[#668074]">Ganho é uma etapa da simulação</p><p className="mt-1 text-xs leading-5 text-[#71887D]">A etapa de fechamento aparece na lista acima e possui sua própria taxa, cor e nome. Ajuste esses campos para testar cenários sem depender de um resultado automático separado.</p></div></div>
    </div>
  </section>;
}
...
function ProspectingWorkspace({ lists, trashedLists, funnels, activeListId, activeListName, prospects, onSelectList, onCreateList, onRenameList, onDeleteList, onRestoreList, onPermanentDeleteList, onAdd, onUpdate, onDelete, onImportList, onSave, onSaveProspect, isSaving, savingProspectId, syncConfirmed, onConnect }: { lists: ProspectList[]; trashedLists: TrashedProspectList[]; funnels: SalesFunnel[]; activeListId: string; activeListName: string; prospects: ProspectRecord[]; onSelectList: (id: string) => void; onCreateList: () => void; onRenameList: (name: string) => void; onDeleteList: () => void; onRestoreList: (id: string) => void; onPermanentDeleteList: (id: string) => void; onAdd: () => void; onUpdate: (id: string, field: keyof Omit<ProspectRecord, "id">, value: string) => void; onDelete: (id: string) => void; onImportList: (listId: string, funnelId: string, stageId: string) => void; onSave: () => void; onSaveProspect: (id: string) => void; isSaving: boolean; savingProspectId: string | null; syncConfirmed: boolean; onConnect: () => void }) {
  ...
  const columns: Array<{ key: keyof Omit<ProspectRecord, "id">; label: string; width: string; multiline?: boolean }> = [
    { key: "decisionMakerFirstName", label: "Nome do decisor", width: "w-[150px] min-w-[150px]" },
    { key: "decisionMakerLastName", label: "Sobrenome do decisor", width: "w-[160px] min-w-[160px]" },
    { key: "decisionMakerRole", label: "Cargo", width: "w-[135px] min-w-[135px]" },
    { key: "decisionMakerEmail", label: "E-mail do decisor", width: "w-[255px] min-w-[255px]" },
    { key: "decisionMakerPhone", label: "Telefone principal", width: "w-[155px] min-w-[155px]" },
    { key: "decisionMakerSecondaryPhone", label: "Telefone secundário", width: "w-[155px] min-w-[155px]" },
    { key: "monthlyVisits", label: "Visitas mensais", width: "w-[120px] min-w-[120px]" },
    { key: "company", label: "Empresa", width: "w-[170px] min-w-[170px]" },
    { key: "companyWebsite", label: "Site da empresa", width: "w-[190px] min-w-[190px]" },
    { key: "analysis", label: "Análise", width: "w-[270px] min-w-[270px]", multiline: true },
  ];
  ...
  return <div className="min-h-screen bg-[#F6F5F1] px-5 pb-8 pt-[92px] md:px-8 md:pt-8">
    <div className="mx-auto w-full max-w-none">
      ...
      <div className={`mt-3 flex flex-wrap items-center justify-between gap-3 rounded-xl border px-3.5 py-2.5 text-xs ${syncConfirmed ? "border-[#CBE5D7] bg-[#F0F8F3] text-[#087E5A]" : "border-[#E7D5A8] bg-[#FFF9E8] text-[#80621D]"}`}><div className="flex items-center gap-2"><span className={`h-2 w-2 rounded-full ${syncConfirmed ? "bg-[#10A97A]" : "bg-[#C88920]"}`} /><span>{persistenceLabel(syncConfirmed)}</span></div></div>
        ...
        <div className="overflow-x-auto overscroll-x-contain [scrollbar-color:#AAB8AF_#F1F4F1] [scrollbar-width:auto]">
          <table className="w-full min-w-[1785px] table-fixed border-collapse text-left">
            ...
            <tbody>{prospects.map((prospect, index) => <tr key={prospect.id} className="group border-b border-[#E8EDE8] align-top transition hover:bg-[#FBFDFB]">
              ...
              <td className="border-r border-[#EEF2EE] px-2 py-1.5"><Button type="button" variant="outline" onClick={() => onSaveProspect(prospect.id)} disabled={isSaving || savingProspectId === prospect.id} className="mt-1 h-9 gap-1 rounded-lg border-[#C8D9CF] px-2.5 text-xs font-extrabold text-[#087E5A] hover:bg-[#E8F6F0] disabled:opacity-60">{savingProspectId === prospect.id ? <Circle size={14} className="animate-spin" /> : <CheckCircle2 size={14} />}{savingProspectId === prospect.id ? "Salvando" : "Salvar"}</Button></td>
              <td className="px-2 py-1.5"><div className="mt-1 flex items-center gap-1.5"><button type="button" onClick={() => setPendingProspectDeleteId(prospect.id)} className="grid h-9 w-9 place-items-center rounded-lg text-[#A0AAA4] opacity-60 transition hover:bg-[#FCEDEB] hover:text-[#B94D45] group-hover:opacity-100" aria-label={`Excluir linha ${index + 1}`}><Trash2 size={16} /></button></div></td>
            </tr>)}</tbody>
          </table>

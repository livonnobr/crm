...
type GoalSimulationStageRecord = { id: string; workspace_id: string; name: string; color: string; probability: number; projection_mode?: SimulationProjectionMode | null; fixed_value?: number | string | null; product_id?: string | null; conversion_stage_id?: string | null; position: number };
...
const initialGoalSimulationStages: GoalSimulationStage[] = [
  { id: "simulation-entry", name: "Entrada", color: "#77918B", probability: 100, projectionMode: "percentual", fixedValue: 0, productId: "", conversionStageId: "" },
  { id: "simulation-diagnosis", name: "Diagnóstico", color: "#5B8CB2", probability: 50, projectionMode: "percentual", fixedValue: 0, productId: "", conversionStageId: "" },
  { id: "simulation-proposal", name: "Proposta", color: "#B07D3A", probability: 35, projectionMode: "percentual", fixedValue: 0, productId: "", conversionStageId: "" },
  { id: "simulation-won", name: "Ganho", color: "#10A97A", probability: 20, projectionMode: "percentual", fixedValue: 0, productId: "", conversionStageId: "" },
];
  ...
  async function loadCloudWorkspace(currentWorkspaceId: string) {
    ...
    const [{ data: goalRows, error: goalsError }, { data: funnelRows, error: funnelsError }, { data: conversionSettingsRow, error: conversionSettingsError }, { data: goalSimulationStageRows, error: goalSimulationStagesError }, { data: prospectListRows, error: prospectListsError }, { data: prospectRecordRows, error: prospectRecordsError }, { data: cadenceBlockRows, error: cadenceBlocksError }, { data: financeRows, error: financeError }, { data: serviceRows, error: servicesError }] = remoteSnapshot
      ...
      : await Promise.all([
          ...
          client.from("goal_simulation_stages").select("id, workspace_id, name, color, probability, projection_mode, fixed_value, product_id, conversion_stage_id, position").eq("workspace_id", currentWorkspaceId).order("position"),
        ...
        {page === "goals" ? (
          <GoalsWorkspace
            ...
            onAddSimulationStage={() => {
              setIsGoalsSyncConfirmed(false);
              const nextStage: GoalSimulationStage = { id: uniqueId("goal-simulation-stage"), name: "Nova etapa", color: "#10A97A", probability: 50, projectionMode: "percentual", fixedValue: 0, productId: "", conversionStageId: "" };
            ...
            onUpdateSimulationStage={(id, field, value) => {
              ...
              setGoalSimulationStages((current) => current.map((stage) => {
                ...
                const nextValue = field === "probability" ? Math.min(100, Math.max(0, Number(value) || 0)) : field === "fixedValue" ? Math.max(0, Number(value) || 0) : value;
    ...
    <div className="pt-[68px] md:pt-0">
      ...
      <div className="px-5 pb-12 md:px-10">
        <section className="instrument-strip" aria-label="Instrumentos de acompanhamento comercial">
          ...
          <div className="instrument-cell order-first min-w-0 border-[#E5BE6B] bg-[#FFF4D9] shadow-[0_10px_24px_rgba(196,141,35,0.12)] sm:col-span-2 xl:col-start-3 xl:col-span-2 xl:row-start-1"><div className="flex items-center gap-2"><span className="pulse-dot bg-[#C88920]" /><span className="text-[9px] font-extrabold uppercase tracking-[0.1em] text-[#8A651D]">Fechamento do mês</span></div><p className="mt-2 font-display text-[30px] font-extrabold leading-none tracking-[-0.065em] text-[#6B4D12]">{daysRemaining} {daysRemaining === 1 ? "dia útil" : "dias úteis"}</p><p className="mt-1 text-[11px] font-semibold capitalize text-[#8A6D2A]">restantes em {currentMonth}</p><div className="mt-4 rounded-xl border border-[#E7C983] bg-[#FFF8E8] p-3"><div className="grid grid-cols-7 gap-x-2 gap-y-1.5 text-center text-[12px] font-bold text-[#A87520]"><div className="contents">{["S", "T", "Q", "Q", "S", "S", "D"].map((day, index) => <span key={`weekday-${index}`} className={`text-[9px] uppercase tracking-[0.12em] ${index >= 5 ? "font-black text-[#C62828]" : "text-[#8C641D]"}`}>{day}</span>)}</div>{calendar.days.map((day, index) => {
...
function GoalSimulator({ stages, services, funnels, onAddStage, onUpdateStage, onDeleteStage, onSave, isSaving, syncConfirmed }: { stages: GoalSimulationStage[]; services: Service[]; funnels: SalesFunnel[]; onAddStage: () => void; onUpdateStage: (id: string, field: "name" | "color" | "probability" | "projectionMode" | "fixedValue" | "productId" | "conversionStageId", value: string | number) => void; onDeleteStage: (id: string) => void; onSave: () => void | Promise<void>; isSaving: boolean; syncConfirmed: boolean }) {
  ...
  const projections = useMemo(() => {
    ...
    return stages.map((stage, index) => {
      ...
      const rate = mode === "produto" ? conversionStage?.probability ?? 0 : index === 0 ? 100 : stage.probability;
      const volume = index === 0 ? simulationBase : mode === "fixo" ? Math.max(0, Number(stage.fixedValue) || 0) : previousVolume * (rate / 100);
      const projected = mode === "produto" && product && conversionStage ? volume * product.price : volume;
      previousVolume = volume;
      return { stage, mode, rate, volume, projected, displayUnit: mode === "produto" && product && conversionStage ? "currency" as const : "volume" as const, product, conversionStage };
    });
